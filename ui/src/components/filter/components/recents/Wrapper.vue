...
<script setup lang="ts">
    ...
    import {getRecents, removeRecent, getSaved, removeSaved} from "../../filters";
    ...
    const loadAll = () => {
        recents.value = getRecents(props.prefix).reverse();
        saved.value = getSaved(props.prefix).reverse();
    };
    ...
    const remove = (prefix, index) => {
        if (prefix === "recents") {
            removeRecent(props.prefix, recents.value[index]);
            recents.value.splice(index, 1);
        } else if (prefix === "saved") {
            removeSaved(props.prefix, saved.value[index]);
